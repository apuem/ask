...
function loadQuestion() {
  ...
  if (questionById == true) {
    question = httpGet(
      "https://ask-api.vercel.app/api/getQuestionById/" + questionId.toString()
    );
  } else {
    question = httpGet(
      "https://ask-api.vercel.app/api/getQuestionByCategory/" + questionCategory
    );
  }
...
function setStatus(answerField, status, activator) {
  if (activator == true) {
    switch (status) {
      case "correct":
        var answerFieldElement = document.getElementById(
          "answer" + answerField
        );
...
function sendEmail() {
  ...
  emailjs.send("service_uf52i0f", "template_lvps758", templateParams).then(
    function(response) {
